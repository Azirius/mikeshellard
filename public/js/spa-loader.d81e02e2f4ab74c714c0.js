...
function removeActiveClassFromParentListItems() {
    ...
    console.log($('#app-container > nav > div > div.navbar-start > a.navbar-item.is-active').innerHTML);
    $('#app-container > nav > div > div.navbar-start > a.navbar-item.is-active').removeClass('is-active');
}

function addActiveClassToParentListItem(element) {
    // $(element).parent().filter('li').addClass('is-active');
    $(element).filter('li').addClass('is-active');
}
